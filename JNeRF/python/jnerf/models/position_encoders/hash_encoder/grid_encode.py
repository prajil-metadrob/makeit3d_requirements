...
class GridEncode(Function):
    ...
    def __init__(self, hash_func_header, aabb_scale=1, n_pos_dims=3, n_features_per_level=2, n_levels=16,
        ...
        self.m_positions_list = []
        self.m_encoded_positions_list = []


    def execute(self, x, m_grid, is_grad):
        num_elements = x.shape[0]
        m_positions = jt.empty([num_elements * self.n_pos_dims * 2], 'float')
        m_encoded_positions = jt.empty(
            [num_elements * self.m_n_features], self.grad_type)
        assert (m_grid.dtype == self.grad_type)
        assert (m_encoded_positions.dtype == self.grad_type)

        output = jt.empty([num_elements, 32], self.grad_type)
        if is_grad:
            m_grid.requires_grad = True
            output.requires_grad = True
        else:
            output.requires_grad = False
        output, m_positions, m_encoded_positions = jt.code([self.m_hashmap_offsets_table, x, m_grid],
                                                           [output, m_positions,
                                                            m_encoded_positions],
...
""")
        ...
        m_positions = m_positions.detach()
        # print("forward")
        # print(self.m_positions.mean())
        if is_grad:
            self.m_positions_list.append(m_positions)
        m_encoded_positions = m_encoded_positions.detach()
        if is_grad:
            self.m_encoded_positions_list.append(m_encoded_positions)
        # print("output shape", output.shape)
        return output

    def grad(self, grad_x):
        # if len(self.num_list) != 0:
        # debug = self.debug_list.pop(0)
        # print(debug)
        # print(grad_x.sum())
        m_grid_gradient = jt.empty([self.m_n_params], self.grad_type)
        m_positions = self.m_positions_list.pop(0)

        m_encoded_positions = self.m_encoded_positions_list.pop(0)

        m_grid_gradient, m_encoded_positions = \
            jt.code([m_positions, self.m_hashmap_offsets_table, grad_x],
                    [m_grid_gradient, m_encoded_positions],
        ...
        m_grid_gradient.compile_options = proj_options
        m_positions = m_positions.detach()

        m_encoded_positions = m_encoded_positions.detach()
        # print("backward")
        # print(m_positions_tmp.mean())
        return None, m_grid_gradient, None